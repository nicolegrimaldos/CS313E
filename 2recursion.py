...
# with "or" you just need one of them to work
...
def groupNoAdj(start, nums, target):
    if (start >= len(nums)):
        ...
        return groupSum(start + 2, nums, target - nums[start]) or groupSum(start + 1, nums, target)
...
def splitArrayHelper(left_sum, right_sum, idx, nums):
    if idx >= len(nums):
        return left_sum == right_sum
    else:
        return splitArrayHelper(left_sum + nums[idx], right_sum, idx + 1, nums) or \
               splitArrayHelper(left_sum, right_sum + nums[idx], idx + 1, nums)
